...
class Main:
    ...
    def __init__(self, input_path='', output_path='', suffix=''):
        ...
        self.top10classifiers = ["AdaBoostClassifier", "DecisionTreeClassifier",   "ExtraTreeClassifier",  "GaussianNB",
                                 "KNeighborsClassifier", "SVC", "LogisticRegression", "RandomForestClassifier",  "XGBClassifier"]
    ...
    def select_classifiers(self):
        ...
        for name, model in CLASSIFIERS:
            ...
            if name in self.top10classifiers:
                # i = i + 1
    ...
    def run(self):
        ...
        y_train = np.array(train['label'].tolist())  # np.array()
        X_train = np.array(train.drop(labels, axis=1))
        clf = LazyClassifier(verbose=0, ignore_warnings=True, custom_metric=None,
                             predictions=True, classifiers=self.select_classifiers())
